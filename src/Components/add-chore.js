...
export default class AddChore extends React.Component {
  ...
  handleSubmit(event) {
    ...
    if (this.context.chores.find((chore) => chore.title === this.state.title)) {
      window.alert(
        `${title} already exists!  If you want to add multiple of the same chore just add a number ex. take out trash 2`
      );
    } else {
